...
public class OptimisticLock {
    ...
    public static void invoke(int Avalue, String i)
        ...
        System.out.println("A的版本  "+Avalue + ": 系统版本"+ value );

        if (Avalue != value) {//判断value版本
            System.out.println( " A版本不一致,不执行");
            value--;
        } else {
            Avalue++;//对数据操作
            value = Avalue;//对数据操作
            System.out.println("A:利用value运算,value=" + value);
        }
    }
    ...
    public static void invoke2(int Bvalue, String i)
            throws InterruptedException {
        Thread.sleep(1000L);//延长执行时间
        System.out.println("B的版本  "+Bvalue + ": 系统版本"+ value );
        if (Bvalue != value) {//判断value版本
            System.out.println("    B版本不一致,不执行");
        } else {
